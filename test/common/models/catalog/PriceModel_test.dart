...
void main() {
  ...
  test("When no currency then default currency is Euro", () {
    final PriceModel price =
        PriceModel.forAmount(Random().nextInt(3).toDouble());
  ...
  test("When having same value and diff currency then not equal", () {
    final PriceModel price1 =
        PriceModel(amount: 1.0, currency: PriceModel.DEFAULT_CURRENCY);
  ...
  test("When having diff value then not equal", () {
    final PriceModel price1 =
        PriceModel(amount: 1.0, currency: PriceModel.DEFAULT_CURRENCY);
    final PriceModel price2 =
        PriceModel(amount: 2.0, currency: PriceModel.DEFAULT_CURRENCY);
  ...
  test("When formatting to string then displaying currency", () {
    final PriceModel price =
        PriceModel(amount: 99.99, currency: PriceModel.DEFAULT_CURRENCY);
  ...
  test("When formatting to string then setting 2 decimal positions", () {
    ...
    expect(
        price
            .toString()
            .substring(price.toString().indexOf(".") + 1,
                price.toString().indexOf(" "))
            .length,
        PriceModel.NUM_DECIMALS);
  });
}
